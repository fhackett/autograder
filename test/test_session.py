...
def test__session___init__(backend):
    session = autograder.Session([backend], [], [], {'a': mock.sentinel.a, 'b': mock.sentinel.b})
...
def test__session_run_invididual(OrderedDict, TemporaryDirectory, reporter, backend, action1, action2):
    ...
    assert session.run_individual(mock.sentinel.id) == data
    ...
    assert reporter.on_individual_completion.call_args_list == [
        mock.call(mock.sentinel.id, False, data),
    ]
